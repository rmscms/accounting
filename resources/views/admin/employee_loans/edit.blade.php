...
@section('title', trans('accounting::accounting.employee_loans.edit_title'))
@section('content')
@php
    $disbursementDate = \RMS\Accounting\Support\AccountingDateUi::gregorianYmdToInputDisplay((string) $loan->disbursement_date?->format('Y-m-d'));
    $firstDueDate = \RMS\Accounting\Support\AccountingDateUi::gregorianYmdToInputDisplay((string) $loan->first_due_date?->format('Y-m-d'));
@endphp
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0">{{ trans('accounting::accounting.employee_loans.edit_title') }} - {{ $loan->loan_number }}</h4>
        <a href="{{ route('admin.accounting.employee-loans.show', $loan->id) }}" class="btn btn-light">{{ trans('accounting::accounting.common.back') }}</a>
    </div>

    <form method="post" action="{{ route('admin.accounting.employee-loans.update', $loan->id) }}" data-decimal-places="{{ (int) ($decimalPlaces ?? 0) }}">
        @csrf
        @method('PUT')
        <div class="card">
                ...
                <div class="col-md-4">
                    ...
                    <select name="employee_id" class="form-select" required>
                        ...
                        @foreach($employees as $employee)
                            <option value="{{ $employee->id }}" @selected((int) old('employee_id', $loan->employee_id) === (int) $employee->id)>{{ $employee->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
                    ...
                    <select name="disbursement_bank_id" class="form-select" required>
                        ...
                        @foreach($banks as $bank)
                            <option value="{{ $bank->id }}" @selected((int) old('disbursement_bank_id', $loan->disbursement_bank_id) === (int) $bank->id)>{{ $bank->label_for_select }}</option>
                        @endforeach
                    </select>
                </div>
                <x-accounting::date-field name="disbursement_date" :label="trans('accounting::accounting.employee_loans.fields.disbursement_date')" :value="old('disbursement_date', $disbursementDate)" :required="true" col-class="col-md-2" />
                <x-accounting::date-field name="first_due_date" :label="trans('accounting::accounting.employee_loans.fields.first_due_date')" :value="old('first_due_date', $firstDueDate)" :required="true" col-class="col-md-2" />

                <div class="col-md-4">
                    <label class="form-label">{{ trans('accounting::accounting.employee_loans.fields.principal_amount') }}</label>
                    <input type="text" inputmode="decimal" name="principal_amount" class="form-control js-accounting-amount-input js-loan-principal" value="{{ old('principal_amount', number_format((float) $loan->principal_amount, 0, '.', ',')) }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label">{{ trans('accounting::accounting.employee_loans.fields.annual_interest_rate') }}</label>
                    <input type="number" step="0.0001" min="0" max="100" name="annual_interest_rate" class="form-control js-loan-rate" value="{{ old('annual_interest_rate', $loan->annual_interest_rate) }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">{{ trans('accounting::accounting.employee_loans.fields.installments_count') }}</label>
                    <input type="number" min="1" max="240" name="installments_count" class="form-control js-loan-months" value="{{ old('installments_count', $loan->installments_count) }}" required>
                </div>
            ...
            <div class="card-footer text-end">
                <button type="submit" class="btn btn-primary">{{ trans('accounting::accounting.employee_loans.actions.update') }}</button>
            </div>
        </div>
    </form>
</div>
