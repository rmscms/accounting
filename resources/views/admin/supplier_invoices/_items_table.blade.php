@php
    ...
    $storeUrl = route('admin.accounting.supplier-invoices.items.store', ['supplier_invoice' => $invoice->getKey()]);
    ...
    $linesCurrency = strtoupper((string) ($invoice->currency_code ?: config('accounting.defaults.currency', 'IRR')));
    $baseCurrency = strtoupper((string) config('accounting.defaults.currency', 'IRR'));
@endphp
<div class="alert alert-info border mb-3 small">
    {{ trans('accounting::accounting.supplier_invoice.items_currency_help', ['currency' => $linesCurrency, 'base_currency' => $baseCurrency]) }}
</div>
@if($locked)
    <div class="alert alert-warning border mb-3">
        {{ trans('accounting::accounting.supplier_invoice.items_locked_document') }}
    </div>
@endif

@if($locked)
    @if($items->isEmpty())
        <div class="alert alert-light border mb-0">{{ trans('accounting::accounting.supplier_invoice.items_empty') }}</div>
    @else
            ...
            <table class="table table-sm table-striped mb-0">
                <thead class="table-light">
                    <tr>
                        ...
                        <th class="text-end">{{ trans('accounting::accounting.supplier_invoice.item_tax_rate') }}</th>
    ...
    @endif
@else
    <div class="supplier-invoice-lines-editor"
        ...
        <div class="form-check form-switch mb-3">
            <input class="form-check-input js-tax-method-toggle" type="checkbox" id="si-tax-method-toggle-{{ $invoice->getKey() }}" {{ $taxMethod === 'inclusive' ? 'checked' : '' }}>
            <label class="form-check-label" for="si-tax-method-toggle-{{ $invoice->getKey() }}">
                {{ trans('accounting::accounting.supplier_invoice.tax_mode_toggle') }}
            </label>
        </div>
        <div class="alert alert-light border mb-3 si-lines-empty {{ $items->isEmpty() ? '' : 'd-none' }}">{{ trans('accounting::accounting.supplier_invoice.items_empty') }}</div>
        <div class="table-responsive">
            <table class="table table-sm table-striped mb-0 align-middle">
                <thead class="table-light">
                    <tr>
                        ...
                        <th>{{ trans('accounting::accounting.supplier_invoice.item_product') }}</th>
                        <th class="text-end" style="min-width:90px">{{ trans('accounting::accounting.supplier_invoice.item_qty') }}</th>
                        <th class="text-end" style="min-width:110px">{{ trans('accounting::accounting.supplier_invoice.item_unit_price') }}</th>
                        <th class="text-end" style="min-width:100px">{{ trans('accounting::accounting.supplier_invoice.item_discount') }}</th>
                        <th class="text-end" style="min-width:90px">{{ trans('accounting::accounting.supplier_invoice.item_tax_rate') }}</th>
                        <th class="text-end" style="min-width:100px">{{ trans('accounting::accounting.supplier_invoice.item_total') }}</th>
