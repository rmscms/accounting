...
@php
    ...
    foreach ($fields as $fieldInstance) {
        if ($fieldInstance->type === \RMS\Core\Data\Field::PRICE) {
            $hasPriceField = true;
            break;
        }
    }
                        ...
                        <div class="col-lg-9">
                            ...
                            @switch($field->type)
                                ...
                                @case(\RMS\Core\Data\Field::DATE)
                                    @php
                                        $dateCal = \RMS\Accounting\Support\AccountingDateUi::calendarModeForDom();
                                        $dateInputType = $dateCal === \RMS\Accounting\Support\AccountingDateUi::MODE_GREGORIAN ? 'date' : 'text';
                                        $dateClasses = 'form-control accounting-date-field @error($field->key) is-invalid @enderror';
                                        if ($field->attributes && isset($field->attributes['class'])) {
                                            $dateClasses .= ' ' . $field->attributes['class'];
                                        }
                                        $dateOld = old($field->key);
                                        if ($dateOld !== null && trim((string) $dateOld) !== '') {
                                            $dateDisplayVal = trim(\RMS\Helper\changeNumberToEn((string) $dateOld));
                                        } else {
                                            $fv = $form_values[$field->key] ?? $field->default_value ?? '';
                                            if ($fv instanceof \DateTimeInterface) {
                                                $fv = $fv->format('Y-m-d');
                                            }
                                            $fv = is_string($fv) ? trim($fv) : '';
                                            $dateDisplayVal = $fv === ''
                                                ? ''
                                                : \RMS\Accounting\Support\AccountingDateUi::gregorianYmdToInputDisplay($fv);
                                        }
                                    @endphp
                                    <input type="{{ $dateInputType }}"
                                           name="{{ $field->key }}"
                                           class="{{ $dateClasses }}{{ $dateCal === \RMS\Accounting\Support\AccountingDateUi::MODE_JALALI ? ' persian-datepicker' : '' }}"
                                           data-calendar="{{ $dateCal }}"
                                           @if($dateCal === \RMS\Accounting\Support\AccountingDateUi::MODE_JALALI) data-persian-date data-format="YYYY-MM-DD" @endif
                                ...
                                {{-- PRICE Input --}}
                                @case(\RMS\Core\Data\Field::PRICE)
                                    <input type="text"
                                           name="{{ $field->key }}"
                                           class="form-control amount-field @error($field->key) is-invalid @enderror"
                                           placeholder="{{ $field->placeholder ?? '' }}"
                                           value="{{ old($field->key, $form_values[$field->key] ?? $field->default_value) }}"
                                           autocomplete="off"
                                           @if($field->required) required @endif
...
@push('styles')
@if($hasPriceField)
<link rel="stylesheet" href="{{ asset('admin/plugins/amount-formatter/amount-formatter.css') }}">
@endif
<style>
/* Custom form styles */
.required::after {
...
@push('scripts')
@if($hasPriceField)
    <script src="{{ asset('admin/plugins/amount-formatter/amount-formatter.js') }}"></script>
@endif
<script>
