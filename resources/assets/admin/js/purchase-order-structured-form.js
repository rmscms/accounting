/**
 * سفارش خرید: Select2 AJAX برای supplier_id، بارگذاری فرگمنت اقلام + ویرایشگر خطوط.
 */
(function ($) {
    'use strict';
    ...
    function initFxCard($root) {
        ...
        var rateField = String($card.data('rate-field') || 'fx_rate_at_order');
        var baseAmountField = String($card.data('base-amount-field') || 'amount_base_at_order');
    ...
    function wirePoNumberUniqueness($root) {
        var $inp = $root.find('#fld-po_number');
        if (!$inp.length) {
            return;
        }
        var checkUrl = $inp.data('po-unique-url');
        if (!checkUrl) {
            return;
        }
        var $fb = $root.find('#fld-po_number-unique-feedback');
        var takenMsg = ($root.data('po-msg-taken') || '').toString();
        ...
        function runCheck() {
            ...
            var exclude = $inp.data('po-exclude-id');
            $.ajax({
                url: checkUrl,
                method: 'GET',
                dataType: 'json',
    ...
    function loadItemsFragment($root) {
        var $mount = $root.find('#purchase-order-items-mount');
        if (!$mount.length) {
            return;
        }
    ...
    function boot() {
        var $root = $('.accounting-structured-form[data-form-slug="purchase_orders"]');
        if (!$root.length) {
            return;
        }
